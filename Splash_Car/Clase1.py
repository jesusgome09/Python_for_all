...
class Window(tk.Tk):
    ...
    def cerrar(self):
        if os.path.exists("Splash_Car\Almacen"):
            os.remove("Splash_Car\Almacen")
        else:
    ...
    def buscar(self):
        ...
        if self.entry_mi_id.get() != "":
            if SqliteP.buscar_auto(id__):
                with open("Splash_Car\Almacen", "w") as file_:
                    file_.write(id__)
                self.pack_forget()
                parent = self.master
...
class Rastreo(tk.Frame):  # Todavia me falta mucho
    ...
    def agregar_datos(self, event):
        with open("Splash_Car\Almacen", "r") as file_:
            idd = file_.read()
        ...
        self.label_tiempo_restante.config(text=f"Tiempo restante: {datos[8]}")
