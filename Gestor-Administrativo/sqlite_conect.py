...
def agregar(nombre, apellido, correo_electronico):
    ...
    cursor.execute(f"INSERT INTO usuarios_universal(NOMBRE, APELLIDO, EMAIL) VALUES('{nombre}','{apellido}','{correo_electronico}')")
...
def verificador(nombre, apellido):
    ...
    tam = cursor.execute(f"SELECT id FROM usuarios_universal WHERE NOMBRE='{nombre}' AND APELLIDO='{apellido}'")
...
def crear_tabla(name_table, campo1_key, campo2, campo3, campo4):
    ...
    cursor.execute(f"CREATE TABLE '{name_table}'('{campo1_key}' INTEGER NOT NULL UNIQUE,'{campo2}' TEXT NOT NULL, '{campo3}'TEXT NOT NULL, '{campo4}' TEXT NOT NULL, PRIMARY KEY('{campo1_key}' AUTOINCREMENT))")
...
def devuelve_datos(nombre, apellido):
    ...
    tam = cursor.execute(f"SELECT ID,NOMBRE,APELLIDO,EMAIL FROM usuarios_universal WHERE NOMBRE='{nombre}' AND APELLIDO='{apellido}'")
    resultado = tam.fetchone()
