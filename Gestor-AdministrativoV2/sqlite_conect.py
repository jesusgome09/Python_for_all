...
psa = "Gestor-AdministrativoV2/sqlite_conect.db"
...
def agregar(nombre, apellido, correo_electronico):
    try:
        ...
        print("Lo lamento, a ocurrido un error\nIntentare solucionarlo")

        nonn = connect(psa, isolation_level=None)
        cursor = nonn.cursor()
        cursor.execute(
            f"INSERT INTO usuarios_universal(NOMBRE, APELLIDO, EMAIL) VALUES('{nombre}','{apellido}','{correo_electronico}')"
        )
        ...
        print(
            "No se pudo, por favor intenta verificar los directorios de la base de datos"
        )
